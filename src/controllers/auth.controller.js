    ...
    try {
        ...
        if (!username || !email || !password || !req.file) {
            return res.status(400).json({
                message: "All fields required. username, email, password,img",
            })
        }
        ...
        const uplodCheck = await uploadImg(req.file, 'profiles');

        if (!uplodCheck || !uplodCheck.secure_url) {
            return res.status(500).json({
                message: "Image upload failed in authentication!"
            });
        }
...
export { createUser, loginUser, logOut, forgotPass, resetPassword }